...
3. 用 WD14 tagger 和映射表构建 Pixiv 标题、说明、tag、年龄分级、原创/二创判断
4. 按目标平台打开浏览器并提交
...
`build_pixiv_payload` 可以通过 Pixiv 页面做 live lookup 和 tag 计数，用结果更新 popularity 决策。最终 tag 会按身份、作品/角色、卖点、tagger 分数和 Pixiv 计数排序，再压到 Pixiv 的 10 tag 上限。
