...
LLM reverse 是 Pixiv 上传链路的可选增强层。它只生成 `title_ja`、`title_zh`、`caption_ja`、`caption_zh`，不接管 tag、年龄分级、censor、Civitai 过滤或发布动作。

配置保存在本地私有 `config.json.llm_reverse`：

- `base_url` / `api_key` / `model`：OpenAI 兼容视觉接口。
- `personas`：控制语言、标题风格、简介风格、SFW/NSFW prompt。
- `accounts`：绑定平台、默认 persona、默认 `content_mode`、允许的内容模式。
